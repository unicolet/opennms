 ...
 */
public class MBeansContentPanel extends Panel implements SelectionChangedListener {
	...
	private final Panel attributePanel;
	...
	public MBeansContentPanel(final MBeansController controller) {
		...
		attributePanel.setContent(attributesTable);

		contentLayout = new VerticalLayout();
		contentLayout.setSpacing(true);
		contentLayout.setMargin(true);
		contentLayout.addComponent(nameEditForm);
		contentLayout.addComponent(attributePanel);

		// by default we do not show something
	...
	public void selectionChanged(SelectionChangedEvent event) {
		...
		if (selectedBean instanceof Mbean) {
			...
			attributesTable.modelChanged(selectedBean, controller.getAttributeContainer((Mbean) selectedBean));
			attributePanel.setCaption("MBean Attributes");
		...
		if (selectedBean instanceof CompAttrib) {
			...
			attributesTable.modelChanged(selectedBean, controller.getCompositeMemberContainer((CompAttrib) selectedBean));
			attributePanel.setCaption("Composite Members");
			setContent(contentLayout);
			nameEditForm.selectionChanged(event);
			return;
		}
